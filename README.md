...
1. [ ] 자판기가 가지고 있을 금액을 입력받는다.
  
2. [ ] 입력값을 검증한다.
   - [ ] 숫자인지 검증한다.
   - [ ] 10으로 나누어 떨어지는 지 검증한다.

3. [ ] 해당 금액을 무작위로 동전으로 변환한다.
   - [ ] 동전 단위 중 하나를 랜덤으로 생성한다.
   - [ ] 단위만큼의 금액을 제한 나머지로 과정을 반복한다.
...
9. [ ] 입력값을 검증한다.
   - [ ] 해당 제품이 자판기에 있는 지 검증한다.
   - [ ] 투입한 금액이 상품의 가격보다 큰 지 검증한다.
