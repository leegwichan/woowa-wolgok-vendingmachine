...
1. [X] 자판기가 가지고 있을 금액을 입력받는다.
  
2. [X] 입력값을 검증한다.
   - [X] 숫자인지 검증한다.
   - [X] 10으로 나누어 떨어지는 지 검증한다.

3. [X] 해당 금액을 무작위로 동전으로 변환한다.
   - [X] 동전 단위 중 하나를 랜덤으로 생성한다.
   - [X] 단위만큼의 금액을 제한 나머지로 과정을 반복한다.
...
9. [X] 입력값을 검증한다.
   - [X] 해당 제품이 자판기에 있는 지 검증한다.
   - [X] 투입한 금액이 상품의 가격보다 큰 지 검증한다.



# 미션 - 자판기
- 원 자료 출처 ([우테코 Repository](https://github.com/woowacourse/java-vendingmachine-precourse))
...
- 과제는 해당 레포지토리를 Fork하여 시작한다.
- 미션 구현을 완료한 후 GitHub을 통해 제출해야 한다.
  - 제출 방식은 프리코스에서 했던 방식과 동일하다.
- 과제 구현 시간이 끝난 후, PR을 통해 코드리뷰 실시 예정
...
- 미션은 [java-vendingmachine-precourse](https://github.com/leegwichan/woowa-wolgok-vendingmachine) 저장소를 Fork/Clone해 시작한다.
